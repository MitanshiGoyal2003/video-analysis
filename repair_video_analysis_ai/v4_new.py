...
class RepairAdvisor:
    ...
    # System prompt with strict rules (kept separate for clarity)
    SYSTEM_PROMPT = """You are an expert HVAC and appliance repair advisor.

IMPORTANT RULES:
- Do NOT infer or assume facts that are not explicitly present in the input.
- Do NOT guess causes, timelines, or intent.
- If evidence is missing or unclear, explicitly mark it as "insufficient_evidence".
- Base all conclusions strictly on the provided structured inputs.
- Output must be valid JSON only. No prose.
- Use only the allowed fields and values defined below.
- This output will be reviewed in audits and disputes.

Your analysis must be factual, evidence-based, and traceable to the input data."""
    ...
    def _build_prompt(self, vision_summary: str, audio_transcript: Optional[str] = None) -> str:
        """Build prompt for Gemini with strict system rules."""
        
        parts = [
            self.SYSTEM_PROMPT,  # ADD SYSTEM PROMPT FIRST
            "",
            "="*80,
            "INPUT DATA",
            "="*80,
        ...
        parts.extend([
            ...
            '    "device_type": "string (AC Unit / Washing Machine / Unknown)",',
            '    "confidence": "string (high / medium / low)",',
            '    "evidence": "string (cite specific detections)"',
            "  },",
            '  "issue_severity": {',
            '    "level": "string (Critical / High / Medium / Low / insufficient_evidence)",',
            '    "evidence": "string (cite damage classifications)"',
            "  },",
            '  "root_cause_analysis": {',
            '    "primary_cause": "string (or insufficient_evidence)",',
            '    "contributing_factors": ["string"],',
            '    "evidence": "string (cite only what is present in data)"',
            "  },",
            '  "repair_action_plan": {',
            '    "immediate_actions": ["string"],',
            '    "parts_replacement": ["string (only if damage detected)"],',
            '    "required_tools": ["string"],',
            '    "evidence": "string"',
            "  },",
            '  "approval_recommendation": {',
            '    "decision": "string (YES / NO / REVIEW / insufficient_evidence)",',
            '    "justification": "string (must reference specific evidence)",',
            '    "conditions": ["string (if decision is REVIEW)"]',
            "  }",
            "}",
            "",
            "="*80,
            ...
            "- Use 'insufficient_evidence' when data is missing or unclear",
            "- Do NOT assume information not present in input",
            "- All justifications must cite specific evidence from the input",
            "="*80
        ])
...
def process_vision_json(json_file: str, 
    ...
    if recommendations.get('finish_reason') == 'MAX_TOKENS':
        print("\n⚠️  WARNING: LLM response may have been truncated due to token limits.")
